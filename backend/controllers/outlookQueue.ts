...
async function init(body: { from: string; to: string; id: string; token: string }) {
...
const sendOutlookMailViaQueue = async (req: Request, res: Response) => {
  try {
...
const sendoutlookEmail = async (data:any, token: string) => {
    console.log(data)
  try {
    if (!token) {
...
const parseAndSendoutlookMail = async (data1: parseContent, token: string) => {
