...
interface EmailData {
  ...
  async function init(body: EmailData) {
  ...
  const sendMailViaQueue = async (req: Request, res: Response) => {
    try {
      ...
      log.info('Error in sending mail via queue', error.message);
    }
    res.send('Mail processing has been queued.');
  };
  
  const sendMultipleEmails = async (req: Request, res: Response) => {
    try {
      const { id } = req.params;
      const { from, to } = req.body;
      ...
      log.error('Error in sending multiple emails', error.message);
      res.status(500).send('Error in sending multiple emails');
    }
  };
  
  async function sendEmailToQueue({ from, to, id }: { from: string; to: string; id: string }) {
    try {
      // Enqueue a job to send the email
      await sendMailQueue.add('send-email', { from, to, id });
      log.info(`Email to ${to} has been queued.`);
    } catch (error) {
      log.error('Error enqueuing email job:', error.message);
      throw error;
    }
  }
  
 
const sendMail = async (data: Data, token: string) => {
    try {
      if (!token) {
      ...
      if (data.label === "Interested") {
        emailContent = `
          If the email mentions they are interested, do not generate any recipient's name
          instead use Dear user, your reply should give this advertisement i have express some key points
          below user then and create good reply for advertisement it should render on email in bullet points
          <div style="background-color: #f5f5f5; padding: 20px; border-radius: 10px;">
            <p>We're excited to share with you how our product can benefit you:</p>
            <ul>
              <li><strong>Secure Mailing:</strong> Our platform offers end-to-end encryption to ensure your 
              emails remain private and secure.</li>
              <li><strong>Automated Emails:</strong> Easily automate your email workflows by setting 
              timers and triggers. Schedule emails to be sent at specific times or based on user actions.</li>
              <li><strong>Customizable Templates:</strong> Create personalized email templates and automate 
              repetitive tasks, saving you time and effort.</li>
            </ul>
            <p>Would you like to learn more about how our platform can streamline your email communication? 
        ...
        emailContent = `
          If the email mentions they are not interested, create a reply where we should 
          ask them for feedback on why they are not interested. do not generate any recipient's name 
          instead use Dear user.
          Write a small text on the above request in around 100-150 words`;
  
        mailOptions.subject = `User is : ${data.label}`;
      } else if (data.label === "More Information") {
        emailContent = `
          If the email mentions they are interested to know more, your reply should give them more information 
          about this product. Here are some of its key features:<br><br>
          ...
          <p>Our product is a comprehensive email management platform designed to streamline your 
    ...
    const sendMessageResponse = await axios.post(`https://gmail.googleapis.com/gmail/v1/users/${data.from}/messages/send`, 
        {
    ...
    log.info(sendMessageResponse.data.id);
    ...
    interface parceContent{
        from: string;
        to: string;
        id: number;
        
    }
  const parseAndSendMail = async (data1: parceContent, token: string) => {
    try {
      const { from, to } = data1;
      if (!token) {
        throw new Error("Token not found, please login again to get token");
      ...
      const response = await openai.chat.completions.create({
        ...
        messages: [
          {
            role: "user",
            content: `based on the following text  just give one word answer, 
            Categorizing the text based on the content and assign a label from the given options 
            - Interested, Not Interested, More information. text is : ${emailContextUpdated}`,
          },
        ],
      });
      ...
      log.error("Can't fetch email ", error.message);
      return -1;
    }
  };
